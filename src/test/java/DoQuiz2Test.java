...
// ⚠️ DO NOT CHANGE: This interacts with the same statistics.json as DoQuizTest
public class DoQuiz2Test { 
    ...
    private static final String DATA_FILE = "statistics.json"; // SHARED FILE
        ...
        try (Playwright playwright = Playwright.create()) {
            ...
            while (true) {
                try {
                    ...
                    sub2.waitFor();
                    sub2.selectOption(new SelectOption().setIndex(3)); // Picks Category 2, Subject 3

                    // CRITICAL: Triggers the site's logic to refresh the quiz ID
                    page.evalOnSelector("#subcategory-2", "el => el.dispatchEvent(new Event('change', { bubbles: true }))");

                    String subject2 = sub2.evaluate("el => el.options[el.selectedIndex].text").toString();
                    System.out.println("🎯 TARGETING SUBJECT: " + subject2);
    ...
    private static void processQuestion(FrameLocator quizFrame, Page page, int i) throws Exception {
        ...
        // Check if ANY subject (English, Math, etc) has ever seen this question
        if (masterDatabase.containsKey(qText)) {
            choiceToClick = masterDatabase.get(qText);
            System.out.println("📝 Q" + i + " [Master Memory]: " + choiceToClick);
        } else {
            ...
            if (resultText.contains("Correct:")) {
                ...
                if (correctIndex >= 0 && correctIndex < options.size()) {
                    ...
                    if (!masterDatabase.containsKey(qText)) {
                        ...
                        saveData(); // Save immediately so other bots see it
    ...
    // --- SHARED DATA LOGIC: Interacts with the same file as DoQuizTest ---
