...
public class DoQuizTest {
    ...
    // AI remains initialized but bypassed for "Data Mining" mode
    ...
    public static void main(String[] args) throws IOException {
        ...
        System.out.println("👤 Account: " + profileName + " | Memory Size: " + masterDatabase.size());
        ...
        try (Playwright playwright = Playwright.create()) {
            ...
            while (true) {
                try {
                    System.out.println("\n🚀 [" + new Date() + "] NEW BATCH | Profile: " + profileName);

                    if (page.isClosed()) page = context.newPage();

                    page = loginIfNeeded(page, context, profileName);                  
                    page.navigate("https://www.iwacusoft.com/ubumenyibwanjye/index",
                            new Page.NavigateOptions().setWaitUntil(WaitUntilState.DOMCONTENTLOADED));

                    // --- NAVIGATION & RANDOM SUBCATEGORY SELECTION (1 to 5) ---
                    page.locator("button:has-text('START EARN')").click();
                    
                    for (int level = 1; level <= 5; level++) {
                        String selector = "#subcategory-" + level;
                        Locator dropdown = page.locator(selector);
                        
                        // Wait a moment for the dropdown to populate from the previous choice
                        page.waitForTimeout(1000); 

                        if (dropdown.isVisible() && dropdown.isEnabled()) {
                            // Get number of options in this dropdown
                            int optionCount = (int) dropdown.locator("option").count();
                            if (optionCount > 1) {
                                // Pick random index (skipping index 0 if it's "Select Category")
                                int randomIndex = random.nextInt(optionCount - 1) + 1;
                                dropdown.selectOption(new SelectOption().setIndex(randomIndex));
                                System.out.println("📂 Level " + level + " selected index: " + randomIndex);
                            }
                        } else {
                            // If the level doesn't exist, we've reached the end of the category chain
                            break; 
                        }
                    }
                    ...
                    while (currentQuestion <= totalQuestions) {
                        try {
                            ...
                            System.err.println("⚠️ Q" + currentQuestion + " fail. Reloading...");
                            page.reload();
                    ...
                    saveData();
                    System.out.println("🏁 Round finished. Statistics synced.");
    ...
    private static void processQuestion(FrameLocator quizFrame, Page page, GroqService ai, int i) throws Exception {
        ...
        // PRIORITIZE MEMORY -> THEN RANDOM (FOR DATA MINING)
        if (masterDatabase.containsKey(qText)) {
            finalChoice = masterDatabase.get(qText);
            System.out.println("📝 Q" + i + " [MEMORY]: " + finalChoice);
        } else {
            int randomIndex = random.nextInt(options.size());
            finalChoice = options.get(randomIndex);
            System.out.println("📝 Q" + i + " [NEW]: Guessing to reveal answer...");
        }
        ...
        // --- TRUTH EXTRACTION ---
        try {
            quizFrame.locator("#lastBody").waitFor(new Locator.WaitForOptions().setTimeout(4000));
    ...
    private static Page loginIfNeeded(Page page, BrowserContext context, String profileName) {
        try {
            page.navigate("https://www.iwacusoft.com/login");
...
//     private static final List<Integer> SUBCATEGORIES = Arrays.asList(1, 2, 3, 4, 5);
//     private static final Set<Integer> assignedSubcategories = new HashSet<>();
...
//                     // ==================== SELECT RANDOM UNIQUE SUBCATEGORY ====================
//                     int selectedSubcategory = getRandomSubcategory();
//                     String subcategoryLocator = "#subcategory-" + selectedSubcategory;
//                     page.locator(subcategoryLocator).waitFor();
//                     page.selectOption(subcategoryLocator, new SelectOption().setIndex(0));
...
//                                     }
//                                 }
//                                 processQuestion(quizFrameRetry, page, currentQuestion, questionStartTime);
//                                 success = true;
//                                 currentQuestion++;
//                             } catch (Exception e) {
//                                 System.err.println("⚠️ Retry Q" + currentQuestion +
...
//     // ==================== RANDOM UNIQUE SUBCATEGORY SELECTION ====================
//     private static int getRandomSubcategory() {
//         List<Integer> available = new ArrayList<>();
//         for (int s : SUBCATEGORIES) if (!assignedSubcategories.contains(s)) available.add(s);

//         if (available.isEmpty()) {
//             assignedSubcategories.clear(); // reset if all used
//             available.addAll(SUBCATEGORIES);
//         }

//         int chosen = available.get(random.nextInt(available.size()));
//         assignedSubcategories.add(chosen);
//         System.out.println("🎯 Selected subcategory: " + chosen);
//         return chosen;
//     }
...
//     // ==================== PROCESS QUESTION ====================
//     private static void processQuestion(FrameLocator quizFrame, Page page, int i, long questionStartTime) throws Exception {
...
//             System.out.println("📝 Q" + i + " [Memory] " + finalChoice);
//         } else {
//             finalChoice = options.get(random.nextInt(options.size())); // random pick
//             System.out.println("📝 Q" + i + " [Random Pick] " + finalChoice);
//         }
